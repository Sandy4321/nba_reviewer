	...
	def write_game(self, data):
		...
		if games:
			...
			for game in games:
				...
				try:
					game_id = None
					self.cur.execute('SELECT game_id FROM games_game WHERE game_id = \'%s\'' % game['id'])
					game_id = self.cur.fetchone() # self.cur.fetchall()

					if game_id:
						print 'Update Game'
						self.cur.execute('UPDATE games_game SET game_id = \'%s\' WHERE game_id = \'%s\'' % (game['id'], game['id']))
					else:
						print 'Insert Game'
						self.cur.execute('INSERT INTO games_game (game_id, home_id, away_id) VALUES (\'%s\', \'%s\', \'%s\')' % 
...
end_date = datetime.date(2014, 10, 30)

games = NBAGames()
games.import_all_teams()
games.import_all_games(start_date, end_date)